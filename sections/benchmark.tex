...
SolidBench's workload combines standard SNB \textit{interactive} queries with custom \textit{discover} templates,
which explicitly target these Link Traversal Query Processing (LTQP) choke points in decentralized environments.

To support sequence-based evaluation, we extend three core components of the SolidBench ecosystem:
\rt{We should make sure to properly explain SolidBench and its architecture (we can do that now, with the space limitations of a journal).}
