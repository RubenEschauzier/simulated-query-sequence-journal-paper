...
and applies the Linked Data Benchmark Council (LDBC) choke point methodology~\cite{angles2014linked}\rt{We should explain this methodology (somewhere).}.
...
To support sequence-based evaluation, we extend three core components of the SolidBench ecosystem:\rt{We should make sure to properly explain SolidBench and its architecture (we can do that now, with the space limitations of a journal).}
...
\rt{I would use sub-listings to split different files instead of comments.}
\begin{lstlisting}[
    language=json,
