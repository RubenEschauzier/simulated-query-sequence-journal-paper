...
\paragraph{Quad store Cache}
This approach uses a quad store to unify cache indexes, saving memory and improving search efficiency.
The quad graph stores the source of the triples, allowing cache hits to retrieve data using an `spo' query where the graph term matches the cached document's URL.
...
We evaluate two cardinality estimation approaches using cached quads to address this. 
The default Comunica cost-based optimizer~\cite{taelman2018comunica} uses these estimates to generate more accurate query plans,
...
For our experiments we evaluate three cache capacities: small (350k quads, $\sim$10\% of the dataset), 
...
Ultimately, comparing these strategies establishes $10$ strong baselines 
